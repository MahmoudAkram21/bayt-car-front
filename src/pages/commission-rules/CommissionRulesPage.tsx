...
const SCOPE_OPTIONS: { value: CommissionRuleScope; labelKey: string; icon: any }[] = [
  { value: 'GLOBAL', labelKey: 'scopeGlobal', icon: Globe },
  { value: 'SERVICE', labelKey: 'scopeService', icon: Layers },
  { value: 'PROVIDER', labelKey: 'scopeProvider', icon: Briefcase },
];
...
export const CommissionRulesPage = () => {
  ...
  const scopeLabel: Record<CommissionRuleScope, string> = {
    GLOBAL: t('commissionRulesPage.scopeGlobal'),
    SERVICE: t('commissionRulesPage.scopeService'),
    PROVIDER: t('commissionRulesPage.scopeProvider'),
  };
  ...
  const handleDelete = (id: number) => {
    if (window.confirm(t('commissionRulesPage.deleteConfirm'))) deleteMutation.mutate(id);
  };
    ...
    <div className="animate-fade-in space-y-6">
        ...
        <div className="flex items-center gap-3">
           ...
           <div>
             <h1 className="text-3xl font-bold tracking-tight text-gray-900 dark:text-white sm:text-4xl">
               {t('commissionRulesPage.title')}
             </h1>
             <p className="mt-1 text-base text-gray-600 dark:text-gray-400">
               {t('commissionRulesPage.description')}
             </p>
           </div>
        </div>
      ...
      {showForm && (
        <Card className="border-amber-100 bg-amber-50/50 backdrop-blur-sm dark:border-amber-900/50 dark:bg-amber-900/10">
          <CardHeader>
            <CardTitle className="text-lg text-gray-900 dark:text-white">{editingId ? t('commissionRulesPage.editRule') : t('commissionRulesPage.addRuleNew')}</CardTitle>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="grid gap-6 sm:grid-cols-2 lg:grid-cols-6">
              ...
              <div className="lg:col-span-2">
                <Label>{t('commissionRulesPage.scope')}</Label>
                <div className="relative mt-1">
                  ...
                  >
                    {SCOPE_OPTIONS.map((o) => (
                      <option key={o.value} value={o.value}>{t(`commissionRulesPage.${o.labelKey}`)}</option>
                    ))}
                  </select>
                </div>
              </div>
              <div className="lg:col-span-1">
                <Label>{t('commissionRulesPage.entityId')}</Label>
                <Input type="number" min="0" placeholder={t('commissionRulesPage.entityIdPlaceholder')} value={form.entity_id} onChange={(e) => setForm((p) => ({ ...p, entity_id: e.target.value }))} className="mt-1 bg-white dark:bg-gray-800" />
              </div>
              <div className="lg:col-span-1">
                <Label>{t('commissionRulesPage.customerCommissionPct')}</Label>
                <div className="relative mt-1">
              ...
              <div className="lg:col-span-1">
                <Label>{t('commissionRulesPage.providerCommissionPct')}</Label>
                <div className="relative mt-1">
              ...
              <div className="flex items-center pt-6 lg:col-span-1">
                <label className="flex items-center gap-2 text-sm font-medium text-gray-700 dark:text-gray-300">
                  ...
                  {t('commissionRulesPage.isActive')}
                </label>
              </div>
              <div className="flex items-end gap-2 sm:col-span-2 lg:col-span-6 justify-end">
                ...
                <Button type="submit" size="sm" className="rounded-xl bg-amber-600 hover:bg-amber-700" disabled={(editingId ? !canUpdateRules : !canCreateRules) || createMutation.isPending || updateMutation.isPending}>
                  {editingId ? t('commissionRulesPage.saveChanges') : t('commissionRulesPage.addRuleBtn')}
                </Button>
              </div>
              </fieldset>
            </form>
      ...
      <div className="grid gap-6 sm:grid-cols-3">
        <div className="group overflow-hidden rounded-2xl border border-gray-100 bg-white/60 p-6 shadow-sm backdrop-blur-xl transition-all hover:shadow-lg dark:border-gray-700 dark:bg-gray-800/60">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-500 dark:text-gray-400">{t('commissionRulesPage.totalRules')}</p>
        ...
        <div className="group overflow-hidden rounded-2xl border border-gray-100 bg-white/60 p-6 shadow-sm backdrop-blur-xl transition-all hover:shadow-lg dark:border-gray-700 dark:bg-gray-800/60">
           <div className="flex items-center justify-between">
             <div>
               <p className="text-sm font-medium text-gray-500 dark:text-gray-400">{t('commissionRulesPage.activeRules')}</p>
        ...
        <div className="group overflow-hidden rounded-2xl border border-gray-100 bg-white/60 p-6 shadow-sm backdrop-blur-xl transition-all hover:shadow-lg dark:border-gray-700 dark:bg-gray-800/60">
           <div className="flex items-center justify-between">
             <div>
               <p className="text-sm font-medium text-gray-500 dark:text-gray-400">{t('commissionRulesPage.avgProviderCommission')}</p>
      ...
      <Card className="overflow-hidden rounded-2xl border-gray-200 bg-white/60 shadow-lg backdrop-blur-xl dark:border-gray-700 dark:bg-gray-800/60">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-gray-900 dark:text-white">
            ...
            {t('commissionRulesPage.rulesListTitle')}
          </CardTitle>
          <CardDescription>{t('commissionRulesPage.rulesListDesc')}</CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading && (
          ...
          {error && (
            <div className="py-8 text-center text-red-600 dark:text-red-400">
              {t('commissionRulesPage.loadError')}
            </div>
          )}
          {!isLoading && !error && rules.length === 0 && (
            <div className="flex flex-col items-center justify-center py-12">
              ...
              <p className="mt-4 text-gray-500 dark:text-gray-400">{t('commissionRulesPage.noRules')}</p>
            </div>
          )}
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-gray-100/50 dark:bg-gray-700/40">
                  <tr>
                    <th className="px-6 py-3 text-start text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">{t('commissionRulesPage.scope')}</th>
                    <th className="px-6 py-3 text-start text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">{t('commissionRulesPage.entityId')}</th>
                    <th className="px-6 py-3 text-start text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">{t('commissionRulesPage.customerCommission')}</th>
                    <th className="px-6 py-3 text-start text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">{t('commissionRulesPage.providerCommission')}</th>
                    <th className="px-6 py-3 text-start text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">{t('commissionRulesPage.status')}</th>
                    <th className="px-6 py-3 text-start text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">{t('commissionRulesPage.actions')}</th>
                  </tr>
                </thead>
                    ...
                    <tr key={r.id} className="transition-colors hover:bg-gray-50/50 dark:hover:bg-gray-700/30">
                      ...
                      <td className="px-6 py-4">
                        <span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-medium ${r.is_active ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-300' : 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-400'}`}>
                          ...
                          {r.is_active ? t('commissionRulesPage.active') : t('commissionRulesPage.disabled')}
                        </span>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-1">
                          {canUpdateRules && <Button variant="ghost" size="sm" className="h-8 w-8 rounded-lg p-0 hover:bg-amber-50 hover:text-amber-600 dark:hover:bg-amber-900/20 dark:hover:text-amber-400" onClick={() => startEdit(r)} title={t('commissionRulesPage.edit')}>
                            <Pencil className="h-4 w-4" />
                          </Button>}
                          {canDeleteRules && <Button variant="ghost" size="sm" className="h-8 w-8 rounded-lg p-0 text-red-500 hover:bg-red-50 hover:text-red-700 dark:text-red-400 dark:hover:bg-red-900/20" onClick={() => handleDelete(r.id)} title={t('commissionRulesPage.delete')}>
